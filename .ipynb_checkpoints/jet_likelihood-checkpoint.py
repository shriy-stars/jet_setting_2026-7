...
def log_likelihood(
    ...
    # print("phi1_model finite?", np.all(np.isfinite(phi1_model)))
    # print("phi2_model finite?", np.all(np.isfinite(phi2_model)))
    # print("dist_model finite?", np.all(np.isfinite(dist_model)))
...
def log_prior(prog_pars): #specify some reasonable bounds with a prior function; that way we don't have to brute force a bunch of stuff
    ...
    if 132.0 < ra < 148.0 and -28.0 < dec < -9.0 and 0.5 <dist < 40 and -3.0 < pm_ra < 3.0 and -3.50 < pm_dec < 0.0 and 250.0 < rv < 281.0:
        return 0.0
    return -np.inf
